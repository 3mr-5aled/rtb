...
goto() {{
    ...
    local agent="${{2#-}}"
    local resolved
    resolved="$(command rtb _goto-resolve "$target" 2>/dev/null)"
    if [ -n "$resolved" ] && [ -d "$resolved" ]; then
        cd "$resolved" || return 1
        if [ -n "$agent" ]; then
            command rtb agent "$agent"
        fi
    else
        echo "Project '$target' not found." >&2
        return 1
    fi
}}
rtb() {{
...
function goto
    ...
    if test (count $argv) -ge 2
        set agent (string trim -c '-' $argv[2])
    end
    set -l resolved (command rtb _goto-resolve $target 2>/dev/null)
...
function global:goto {{
    ...
    $bin = if ($env:_RTB_BIN -and (Test-Path $env:_RTB_BIN)) {{ $env:_RTB_BIN }} else {{ (Get-Command rtb -CommandType Application -ErrorAction SilentlyContinue).Source }}
    if (-not $bin) {{ return }}
    $resolved = & $bin _goto-resolve $target 2>$null
    if ($resolved -and (Test-Path $resolved)) {{
        Set-Location $resolved
        if ($agent) {{
            $cleanAgent = $agent.TrimStart('-')
            & $bin agent $cleanAgent
        }}
    }} else {{
        Write-Error "Project '$target' not found."
    }}
}}
function global:rtb {{
    if ($args.Count -gt 0 -and $args[0] -eq 'goto') {{
        $target = if ($args.Count -gt 1) {{ $args[1] }} else {{ $null }}
        $agent = if ($args.Count -gt 2) {{ $args[2] }} else {{ $null }}
        goto $target $agent
    }} else {{
        $bin = if ($env:_RTB_BIN -and (Test-Path $env:_RTB_BIN)) {{ $env:_RTB_BIN }} else {{ (Get-Command rtb -CommandType Application -ErrorAction SilentlyContinue).Source }}
        & $bin @args
...
# Add to your $PROFILE: Invoke-Expression (& rtb shell-init powershell)"#
    ...
    fn execute_goto_resolve(query: String, cli: &Cli) -> Result<i32> {
        ...
        if let Some(p) = projects.iter().find(|p| p.name.to_lowercase().contains(&target)) {
            println!("{}", p.path.display());
            return Ok(0);
        }
        Ok(1)
