...
pub fn draw(f: &mut Frame, app: &App, area: Rect) {
    ...
    let git_projects: Vec<_> = app
        .projects
        .iter()
        .filter(|p| p.git.is_some())
        .collect();

    let dirty_count = git_projects
        .iter()
        .filter(|p| {
            p.git.as_ref().map(|g| g.uncommitted > 0 || g.unpushed > 0).unwrap_or(false)
        })
        .count();
    let local_clean_count = git_projects
        .iter()
        .filter(|p| {
            p.git.as_ref().map(|g| !g.has_remote && g.uncommitted == 0 && g.unpushed == 0).unwrap_or(false)
        })
        .count();
    let synced_count = git_projects.len().saturating_sub(dirty_count + local_clean_count);

    let title = format!(
        " Git Health — {} repos │ ⚠ {} needs attention │ 🏠 {} local clean │ ✅ {} synced ",
        git_projects.len(), dirty_count, local_clean_count, synced_count
    );
