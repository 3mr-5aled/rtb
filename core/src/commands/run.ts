...
export function registerRunCommand(program: Command, getContext: () => CliContext): void {
  program
    ...
    .action(async (projectName: string | undefined, extraArgs: string[] | undefined, options: { dryRun?: boolean }) => {
      ...
      // If args array is passed in options or as extraArgs
      ...
      const resolved = resolveProjectAction('run', targetPath, finalExtraArgs);
      if (!resolved) {
        if (ctx.isJson) {
          outputError(`No runnable script or main entrypoint detected in ${targetPath}`, 'NO_ENTRYPOINT', true);
        } else {
          console.log(chalk.yellow(`\n  ⚠ No runnable script or main entrypoint detected in ${targetPath}\n`));
        }
        process.exit(1);
        return;
      }
      ...
      if (!ctx.isQuiet) {
        ...
        console.log(`\n${chalk.cyan('══════════════════════════════════════════')}`);
        console.log(`  ${chalk.bold(`rtb (رتّب) » Run (${leaf})`)}`);
