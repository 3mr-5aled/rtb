...
export function registerBuildCommand(program: Command, getContext: () => CliContext): void {
  program
    ...
    .action(async (projectName: string | undefined, extraArgs: string[] | undefined, options: { dryRun?: boolean }) => {
      ...
      const resolved = resolveProjectAction('build', targetPath, finalExtraArgs);
      if (!resolved) {
        if (ctx.isJson) {
          outputError(`No build configuration detected in ${targetPath}`, 'NO_BUILD_TARGET', true);
        } else {
          console.log(chalk.yellow(`\n  ⚠ No build configuration detected in ${targetPath}\n`));
        }
        process.exit(1);
        return;
      }
      ...
      if (!ctx.isQuiet) {
        ...
        console.log(`\n${chalk.cyan('══════════════════════════════════════════')}`);
        console.log(`  ${chalk.bold(`rtb (رتّب) » Build (${leaf})`)}`);
